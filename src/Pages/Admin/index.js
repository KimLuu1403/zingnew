...
const Admin = (props) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "center",
      className: "img-table",
      render: (text, record) => <Image src={text} alt="" />,
    },
    {
      title: "Action",
      render: (text, record) => (
  ...
  const handleSwitch = async (value, record) => {
    await updateContentItem({ ...record, isShow: value }); // record là data chung và dùng Operators ES6 thay thế mỗi values của isShow(tham số thứ 2)
    // const params = {
    //   id: record.id,
    //   isShow: values,
    // };
  ...
  return (
    <div className="admin-page">
      ...
      >
        <h3>
          <UserOutlined /> <span>Page</span>
        </h3>
      </div>
      <div className="table-admin">
        <div className="header-admin-page">
          <Button style={{ backgroundColor: "#62aeff" }} onClick={openModal}>
            Create
          </Button>
          <h4>Welcome to admin page and have a beautiful day!</h4>
        </div>
        <Table
        ...
        >
          ...
          >
            ...
            <Form.Item name="isShow" label="Show">
              <Switch onChange={(values) => handleSwitch(values)}></Switch>
            </Form.Item>
            <Form.Item name="Message" label="Message">
              <CKEditor editor={Editor} onInit={(editor) => {}} />
            </Form.Item>
          </Form>
        </Modal>
      </div>
